...
systemd.services = {
  ...
  inotify-scripts = {
    ...
    serviceConfig = {
      ...
      ExecStart = "${pkgs.bash}/bin/bash -c '${pkgs.inotify-tools}/bin/inotifywait -m -r -e modify,create,delete /home/bob/.nix-profile/bin | while read path action file; do bash /home/bob/Documents/scripts/uploadchangesofscriptstogit.sh; done'";
      User = "bob";
      RestartSec = "30s";
      Retart = "on-failure";
